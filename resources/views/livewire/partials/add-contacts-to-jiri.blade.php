...
mount(function (Jiri $jiri, string $role) {
    $this->role = $role;
    if ($this->role === 'student') {
        $this->attendances = $jiri->students;
        $this->modelName = 'étudiant';
    }
    if ($this->role === 'evaluator') {
        $this->attendances = $jiri->evaluators;
        $this->modelName = 'évaluateur';
    }
...
$createAttendance = function (string $role) {
    $this->dispatch('openCreateContactDrawer', role: $this->role, jiri: $this->jiri
    )->to('partials.contacts-drawers');
};


$editAttendance = function (Contact $contact) {
    $this->dispatch('openEditContactDrawer', contact: $contact)->to('partials.contacts-drawers');
};

on(['refreshComponent' => function () {
    $this->mount($this->jiri, $this->role);
}]);
...
<div class=" bg-white border mt-4 shadow-sm ring-1 ring-gray-900/5 p-4">
                ...
                <ul
                    ...
                    @foreach($this->filteredContacts as $contact)
                        <li
                            ...
                            @focus="clearTimeout(blurTimeout); isFocused = true"
                            @blur="blurTimeout = setTimeout(() => { isFocused = false }, 200)"
                            wire:key="{{$role}}_search-{{$contact->id}}"
    ...
    <div class="grid sm:grid-cols-2 xl:grid-cols-4 gap-4
        ...
        @foreach($attendances as $attendance)
            <div class="flex justify-between gap-x-4 py-6 px-4 rounded-xl border border-gray-200">
                ...
                <div class="flex items-center gap-x-2">
                    <button wire:click="deleteFromJiri({{$attendance}})" type="button"
                    ...
                    <button wire:click="editAttendance({{$attendance}}, 'student')" type="button"
                            class="font-semibold text-indigo-600 hover:text-indigo-500">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                             stroke="currentColor" class="size-6">
                            <path stroke-linecap="round" stroke-linejoin="round"
    ...
    <div class="flex border-t border-gray-100 pt-6">
        <button wire:click="createAttendance('{{$role}}')" type="button"
                class="text-sm/6 font-semibold text-indigo-600 hover:text-indigo-500"><span
                aria-hidden="true">+</span> Ajouter un {{$modelName}} qui n'existe pas dans les contacts
        </button>
    </div>
</div>
@script
